...
CREATE TABLE IF NOT EXISTS `Directory_Aziendale`.`Dipendente` (
  ...
  `Data Avvenuto Trasferimento` DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `Directory_Aziendale`.`Dati Dipendente` (
  ...
  `Email Aziendale` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `Directory_Aziendale`.`Trasferimento` (
  `Data Trasferimento` DATE NOT NULL,
...
set transaction isolation level repeatable read; /* non vogliamo che qualcuno cambi la mansione di un dipendente finchè non abbiamo
finito di scandire tutte le tuple dei dipendenti, in modo che il report dei dipendenti da trasferire che otteniamo sia esatto
non mettiamo "serializable" perchè supponiamo che se anche anche viene inserito un nuovo dipendente mentre eseguiamo il report dei 
dipendenti da trasferire, questo non sia da trasferire immediatamente, perchè sarà sicuramente nella sua postazione da meno di 6 mesi, 
e la mansione ad esso assegnata dovrà per forza corrispondere a quella dell'ufficio in cui verrà collocato*/
...
set transaction isolation level repeatable read; /* Non voliamo che possa venir cambiata la mansione di un dipendente mentre
stiamo effettuando un trasferimento, che risulterebbe quindi poi scorretto. Per questo manteniamo il lock sulla tupla per tutta
la durata della transazione */

start transaction;
UPDATE `Dipendente` SET `Dipendente`.`Telefono Esterno` = var_nuovaPostazione, `Data Avvenuto Trasferimento` = curdate()
...
set transaction isolation level repeatable read; /* Non vogliamo che qualcuno possa aggiornare la postazione di un dipendente mentre 
stiamo ancora modificando la sua mansione perchè porterebbe il database in uno stato scorretto, perciò scegliamo una trasazione
repeatable read */
...
set transaction read only;
set transaction isolation level read committed; /* Dopbbiamo solamente leggere le tuple della tabella interessata, perciò
ci basta read committed */

start transaction;
SELECT U.`Nome Ufficio` AS 'Uffici Occupati', T.`Data Trasferimento` as 'Fino al Giorno'
...
SELECT P.`Nome Ufficio`, 'Ufficio Corrente'
FROM `Dipendente` D join `Postazione` P on D.`Telefono Esterno` = P.`Telefono Esterno`
WHERE D.`Codice Fiscale` = var_codiceFiscale;
commit;
...
set transaction read only;
set transaction isolation level read committed; -- come per le altre ricerche non dobbiamo rileggere gli stessi dati
-- quindi basta un livello di isolamento read committed

start transaction;
SELECT `Nome`, `Cognome`, `Codice Fiscale`, `Email Personale`, `Email Aziendale`, D.`Telefono Esterno`, `Nome Ufficio`,
`Mansione Dipendente`, `Numero Piano`, `ID Edificio`
...
set transaction read only;
set transaction isolation level repeatable read; /* Leggiamo più volte gli stessi dati per discriminare i casi in cui
la postazione è libera e quelli in cui invece è occupata (e in questo caso dobbiamo controllare se il dipendente sia da
trasferire o meno, pertanto vogliamo un lock sui dati che leggiamo per tutta la durata della transazione */

start transaction;
if (not exists (select `Codice Fiscale` FROM `Dipendente` WHERE `Telefono Esterno` = var_telefonoEsterno)) then 
SELECT  P.`Nome Ufficio`, U.`Mansione Assegnata` as 'Mansione Ufficio', P.`Numero Piano`, P.`ID Edificio`, P.`Telefono Interno`, 'Si' as 'Postazione Libera'
FROM `Postazione` P join `Ufficio` U on P.`Nome Ufficio` = U.`Nome Ufficio` and P.`Numero Piano` = U.`Numero Piano` and
P.`ID Edificio` = U.`ID Edificio`
WHERE `Telefono Esterno` = var_telefonoEsterno; -- caso postazione non occupata da nessun dipendente
...
if(select var_dipendente in (SELECT D.`Codice Fiscale`
    ...
    select 'No' into var_daTrasferire; -- controllo se il dipendente assegnato alla postazione è da trasferire o meno
end if;

SELECT I.`Nome`, I.`Cognome`, var_daTrasferire as 'Da Trasferire', I.`Email Personale`, I.`Email Aziendale`, D.`Telefono Esterno`,
 P.`Nome Ufficio`, U.`Mansione Assegnata` as 'Mansione Ufficio', P.`Numero Piano`, P.`ID Edificio`, P.`Telefono Interno`
FROM `Dati Dipendente` I join `Dipendente` D on D.`Codice Fiscale` = I.`CF Dipendente` 
join Postazione P on P.`Telefono Esterno` = D.`Telefono Esterno` join `Ufficio` U on P.`Nome Ufficio` = U.`Nome Ufficio` and P.
`Numero Piano` = U.`Numero Piano` and P.`ID Edificio` = U.`ID Edificio`
...
DELIMITER $$
USE `Directory_Aziendale`$$
CREATE PROCEDURE `InserisciDipendente` (in var_cf VARCHAR(45), in var_telPostaz VARCHAR(45), in var_dataTrasf DATE,
...
set transaction isolation level repeatable read; /* Abbiamo dei trigger che fanno dei controlli sui dati inseriti
prima di completare effettivamente l'inserimento. Vogliamo che finchè l'inserimento non sia stato validato le tuple che
ansiamo a leggere non possano essere modificate, per questo scegliamo repeatable read */
start transaction;
...
set transaction isolation level read committed; -- dobbiamo solo eliminare il dipendente, perciò è sufficente un livello più
-- basso di isolamento rispetto a quello standard
...
set transaction isolation level repeatable read; /* I trigger devono controllare gli inserimenti, perciò vogliamo mantenere
il lock sulle tuple che leggiamo fino alla fine della transazione */
...
set transaction read only;
set transaction isolation level read committed;-- abbiamo solamente necessita di leggere una ad una le tuple. Non ci interessa dei 
-- phantom read perchè non facciamo operazioni su gruppi di dipendenti.
...
start transaction;
SELECT `Nome`, `Cognome`, `Codice Fiscale`, `Email Personale`, `Email Aziendale`, P.`Telefono Esterno`, `Nome Ufficio`,
`Mansione Dipendente`, `Numero Piano`, `ID Edificio`
FROM `Dati Dipendente` I join `Dipendente` D on D.`Codice Fiscale` = I.`CF Dipendente` 
join Postazione P on P.`Telefono Esterno` = D.`Telefono Esterno`
...
USE `Directory_Aziendale`$$
DROP TRIGGER IF EXISTS `Directory_Aziendale`.`CompatibiltaMansione` $$
USE `Directory_Aziendale`$$
CREATE TRIGGER `Directory_Aziendale`.`CompatibiltaMansione` BEFORE INSERT ON `Dipendente` FOR EACH ROW
BEGIN

declare var_MansioneUfficio varchar(45);

select `Mansione Assegnata`
from `Postazione` P join `Ufficio` U on P.`Nome Ufficio` = U.`Nome Ufficio` and P.`Numero Piano` = U.`Numero Piano`
and P.`ID Edificio` = U.`ID Edificio`
where P.`Telefono Esterno` = NEW.`Telefono Esterno`
into var_MansioneUfficio;

if (var_MansioneUfficio <> NEW.`Mansione Dipendente`) then
	signal sqlstate '45002' set message_text = 'Mansione nuovo dipendente incompatibile con ufficio';
end if;
...
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipendenteCognome` TO 'DipendenteSettoreSpazi';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipNomeCognome` TO 'DipendenteSettoreSpazi';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`ReportDipendentiTrasferire` TO 'DipendenteSettoreSpazi';
SET SQL_MODE = '';
...
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipendenteNome` TO 'DipendenteSettoreAmministrativo';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaPostazione` TO 'DipendenteSettoreAmministrativo';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipNomeCognome` TO 'DipendenteSettoreAmministrativo';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipendenteCognome` TO 'DipendenteSettoreAmministrativo';
SET SQL_MODE = '';
...
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipNomeCognome` TO 'Dipendente';
GRANT EXECUTE ON procedure `Directory_Aziendale`.`RicercaDipendenteCognome` TO 'Dipendente';
SET SQL_MODE = '';
DROP USER IF EXISTS login;
SET SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
CREATE USER 'login' IDENTIFIED BY 'login';

GRANT EXECUTE ON procedure `Directory_Aziendale`.`login` TO 'login';
...
INSERT INTO `Directory_Aziendale`.`Edificio` (`ID`) VALUES ('Edificio Amministrazione');
...
INSERT INTO `Directory_Aziendale`.`Mansione` (`Nome Mansione`) VALUES ('Spazi');
INSERT INTO `Directory_Aziendale`.`Mansione` (`Nome Mansione`) VALUES ('Amministrativo');
...
INSERT INTO `Directory_Aziendale`.`Piano` (`Numero Piano`, `ID Edificio`) VALUES (3, 'Edificio A');
INSERT INTO `Directory_Aziendale`.`Piano` (`Numero Piano`, `ID Edificio`) VALUES (1, 'Edificio Amministrazione');
...
INSERT INTO `Directory_Aziendale`.`Ufficio` (`Nome Ufficio`, `Numero Piano`, `ID Edificio`, `Mansione Assegnata`) VALUES ('Managment A', 2, 'Edificio A', 'Manager');
INSERT INTO `Directory_Aziendale`.`Ufficio` (`Nome Ufficio`, `Numero Piano`, `ID Edificio`, `Mansione Assegnata`) VALUES ('Ufficio Settore Spazi', 3, 'Edificio A', 'Spazi');
INSERT INTO `Directory_Aziendale`.`Ufficio` (`Nome Ufficio`, `Numero Piano`, `ID Edificio`, `Mansione Assegnata`) VALUES ('Ufficio Amministrativo', 1, 'Edificio Amministrazione', 'Amministrativo');
...
INSERT INTO `Directory_Aziendale`.`Postazione` (`Telefono Esterno`, `Telefono Interno`, `Nome Ufficio`, `Numero Piano`, `ID Edificio`) VALUES ('0652', '333', 'Ufficio Settore Spazi', 3, 'Edificio A');
INSERT INTO `Directory_Aziendale`.`Postazione` (`Telefono Esterno`, `Telefono Interno`, `Nome Ufficio`, `Numero Piano`, `ID Edificio`) VALUES ('999', '121', 'Ufficio Amministrativo', 1, 'Edificio Amministrazione');
INSERT INTO `Directory_Aziendale`.`Postazione` (`Telefono Esterno`, `Telefono Interno`, `Nome Ufficio`, `Numero Piano`, `ID Edificio`) VALUES ('778', '567', 'Managment A', 2, 'Edificio A');
